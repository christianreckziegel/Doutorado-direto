...
std::vector<TH1D*> calculateSelectionEfficiencyRun2Matched(TFile* fClosureInputMatched, const BinningStruct& binningStruct, const std::vector<std::pair<double, double>>& bdtPtCuts) {
    ...
    for (size_t i = 0; i < histCaseNum; ++i) {
        hMcpPt.push_back(new TH1D(Form("mcp_pt_%zu",i), ";#it{p}_{T,D}^{truth};dN/d#it{p}_{T,D}^{truth}", binningStruct.ptHFBinEdges_detector.size() - 1, binningStruct.ptHFBinEdges_detector.data()));
        hMcpPt[i]->SetMarkerColor(kBlack);
        hMcpPt[i]->SetLineColor(kBlack);
        hMcpPt[i]->SetMarkerStyle(kOpenCircle);
        hMcpPt[i]->Sumw2();
        hMcpPt[i]->SetStats(0);
        hMcdPt.push_back(new TH1D(Form("mcd_pt_%zu",i), ";#it{p}_{T,D}^{reco};dN/d#it{p}_{T,D}^{reco}", binningStruct.ptHFBinEdges_detector.size() - 1, binningStruct.ptHFBinEdges_detector.data()));
    ...
    const double MCPHfPtMincut = binningStruct.ptHFBinEdges_particle[0]; // on particle level D0
    const double MCDHfPtMincut = binningStruct.ptHFBinEdges_particle[0]; // on detector level D0
    const double MCPHfPtMaxcut = binningStruct.ptHFBinEdges_particle[binningStruct.ptHFBinEdges_particle.size() - 1]; // on particle level D0
    const double MCDHfPtMaxcut = binningStruct.ptHFBinEdges_detector[binningStruct.ptHFBinEdges_detector.size() - 1]; // on detector level D0
...
EfficiencyData calculateKinematicEfficiency(TFile* fClosureInputMatched, const std::vector<TH1D*>& hSelEff_run2style, const BinningStruct& binningStruct, const std::vector<std::pair<double, double>>& bdtPtCuts) {
    ...
    histStruct.hYieldTruth.first = new TH2D("hYieldTruthPrompt", "Particle level data prompt yield distribution;#it{p}_{T, ch. jet}^{gen} (GeV/#it{c});#it{p}_{T, D^{0}}^{gen} (GeV/#it{c})", 
                                                ...
                                                binningStruct.ptHFBinEdges_particle.size() - 1, binningStruct.ptHFBinEdges_particle.data());
    ...
    histStruct.hYieldMeasured.first = new TH2D("hYieldMeasuredPrompt", "Detector level data prompt yield distribution;#it{p}_{T, ch. jet}^{det} (GeV/#it{c});#it{p}_{T, D^{0}}^{det} (GeV/#it{c})", 
                                                ...
                                                binningStruct.ptHFBinEdges_detector.size() - 1, binningStruct.ptHFBinEdges_detector.data());
    ...
    const double MCPHfPtMincut = binningStruct.ptHFBinEdges_particle[0]; // on particle level D0
    const double MCDHfPtMincut = binningStruct.ptHFBinEdges_detector[0]; // on detector level D0
    const double MCPHfPtMaxcut = binningStruct.ptHFBinEdges_particle[binningStruct.ptHFBinEdges_particle.size() - 1]; // on particle level D0
    const double MCDHfPtMaxcut = binningStruct.ptHFBinEdges_detector[binningStruct.ptHFBinEdges_detector.size() - 1]; // on detector level D0
    TTree* tree = (TTree*)fClosureInputMatched->Get("CorrectionTree");
