...
EfficiencyData createHistograms(const BinningStruct& binning) {
    ...
    for (size_t iCase = 0; iCase < histCaseNum; ++iCase) {
        dataContainer.hMcpPt.push_back(new TH1D(Form("mcp_pt_%zu",iCase), ";#it{p}_{T,D^{0}}^{truth};dN/d#it{p}_{T,D^{0}}^{truth}", binning.ptHFBinEdges_detector.size() - 1, binning.ptHFBinEdges_detector.data()));
