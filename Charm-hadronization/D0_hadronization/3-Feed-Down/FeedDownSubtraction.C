...
void fillHistograms(TFile* fEfficiency, TFile* fPowhegNonPrompt, TFile* fSimulatedMCMatched, TFile* fLumi, FeedDownData& dataContainer, const double& jetptMin, const double& jetptMax, const BinningStruct& binning) {
    ...
    for (int entry = 0; entry < nEntries; ++entry) {
        ...
        // calculating delta R
        double deltaR = sqrt(pow(PowjetEta-PowhfEta,2) + pow(DeltaPhi(PowjetPhi,PowhfPhi),2));
...
std::vector<double> retrieveLuminosityFromFile(TFile* fPowhegNonPrompt, TFile* fLumi, FeedDownData& dataContainer) {
    ...
    TTree* tLumi = nullptr;
    // BC+TVX
    tLumi = (TTree*) fLumi->Get("DF_merged/O2bccount"); // branch -> fCountsWithTVX
    double bcTVX = tLumi->GetEntries();
    // Coll+TVX+VtxZ+Sel8
    tLumi = (TTree*) fLumi->Get("DF_merged/O2collcount"); // branch -> fCountsWithTVXAndZVertexAndSel8
    // TBranch *branch = tLumi->GetBranch("fCountsWithTVXAndZVertexAndSel8");

    tLumi->Draw("fCountsWithTVXAndZVertexAndSel8>>hLumi1");
    TH1D* hLumi1 = (TH1D*)gDirectory->Get("hLumi1");

    double selection = hLumi1->GetMean();
    // Coll+TVX
    // branch = tLumi->GetBranch("fCountsWithTVX");
    tLumi->Draw("fCountsWithTVX>>hLumi2");
    TH1D* hLumi2 = (TH1D*)gDirectory->Get("hLumi2");
    double collTVX = hLumi2->GetMean();
    // Number of TVX triggered BC that correspond to your selections and your train efficiencies
    double triggered = bcTVX * selection / collTVX;
...
void smearGeneratorData(FeedDownData& dataContainer, TFile* fEfficiency, const double& BR) {
...
void plotHistograms(const FeedDownData& dataContainer, const double& jetptMin, const double& jetptMax) {
    ...
    cKinEffParticle->Print(imagePath + Form("feeddown_%.0f_to_%.0fGeV.pdf",jetptMin,jetptMax));
    cKinEffDetector->Print(imagePath + Form("feeddown_%.0f_to_%.0fGeV.pdf",jetptMin,jetptMax));
    ...
    cResponseDeltaR->Print(imagePath + Form("feeddown_%.0f_to_%.0fGeV.pdf",jetptMin,jetptMax));
    cResponsePtJet->Print(imagePath + Form("feeddown_%.0f_to_%.0fGeV.pdf",jetptMin,jetptMax));
    cFedDownData2DOnly->Print(imagePath + Form("feeddown_%.0f_to_%.0fGeV.pdf)",jetptMin,jetptMax));
...
void FeedDownSubtraction(){
    ...
    // D0 -> KPi decay channel branching ratio = (3.93 +- 0.04) %
    double BR = 0.0393;
    ...
    TFile* fLumi = new TFile("../Data/Experimental/Train_643652/AO2Ds_non_merged/AO2D_mergedDFs.root","read");
    ...
    smearGeneratorData(dataContainer, fEfficiency, BR);
