    ...
    for (size_t i = 0; i < ptHFBinEdges.size() - 1; ++i) {
        ...
        if (std::fmod(ptHFBinEdges[i], 1.0) != 0) { // if the first bin edge is not an integer
            if (std::fmod(ptHFBinEdges[i+1], 1.0) != 0) {
                ...
                histograms.reflections.push_back(new TH2D(Form("histMass_reflections%zu",i+1), Form("%.1f < p_{T,D^{0}} < %.1f GeV/c;#it{M}(K#pi) GeV/c^{2};#DeltaR",ptHFBinEdges[i],ptHFBinEdges[i+1]), massBins, minMass, maxMass, deltaRBinEdges.size() - 1, deltaRBinEdges.data()));
                // pure signals histograms
                histograms.signals.push_back(new TH2D(Form("histMass_signals%zu",i+1), Form("%.1f < p_{T,D^{0}} < %.1f GeV/c;#it{M}(K#pi) GeV/c^{2};#DeltaR",ptHFBinEdges[i],ptHFBinEdges[i+1]), massBins, minMass, maxMass, deltaRBinEdges.size() - 1, deltaRBinEdges.data()));
                // signals+reflections histograms: calculate ratios
                histograms.signals_and_reflections.push_back(new TH2D(Form("histMass_signals_and_reflections%zu",i+1), Form("%.1f < p_{T,D^{0}} < %.1f GeV/c;#it{M}(K#pi) GeV/c^{2};#DeltaR",ptHFBinEdges[i],ptHFBinEdges[i+1]), massBins, minMass, maxMass, deltaRBinEdges.size() - 1, deltaRBinEdges.data()));
        
            } else {
                // pure reflections histograms
                histograms.reflections.push_back(new TH2D(Form("histMass_reflections%zu",i+1), Form("%.1f < p_{T,D^{0}} < %.0f GeV/c;#it{M}(K#pi) GeV/c^{2};#DeltaR",ptHFBinEdges[i],ptHFBinEdges[i+1]), massBins, minMass, maxMass, deltaRBinEdges.size() - 1, deltaRBinEdges.data()));
                // pure signals histograms
                histograms.signals.push_back(new TH2D(Form("histMass_signals%zu",i+1), Form("%.1f < p_{T,D^{0}} < %.0f GeV/c;#it{M}(K#pi) GeV/c^{2};#DeltaR",ptHFBinEdges[i],ptHFBinEdges[i+1]), massBins, minMass, maxMass, deltaRBinEdges.size() - 1, deltaRBinEdges.data()));
                // signals+reflections histograms: calculate ratios
                histograms.signals_and_reflections.push_back(new TH2D(Form("histMass_signals_and_reflections%zu",i+1), Form("%.1f < p_{T,D^{0}} < %.0f GeV/c;#it{M}(K#pi) GeV/c^{2};#DeltaR",ptHFBinEdges[i],ptHFBinEdges[i+1]), massBins, minMass, maxMass, deltaRBinEdges.size() - 1, deltaRBinEdges.data()));
        
            }
        } else {
            if (std::fmod(ptHFBinEdges[i+1], 1.0) != 0) {
                // pure reflections histograms
                histograms.reflections.push_back(new TH2D(Form("histMass_reflections%zu",i+1), Form("%.0f < p_{T,D^{0}} < %.1f GeV/c;#it{M}(K#pi) GeV/c^{2};#DeltaR",ptHFBinEdges[i],ptHFBinEdges[i+1]), massBins, minMass, maxMass, deltaRBinEdges.size() - 1, deltaRBinEdges.data()));
                // pure signals histograms
                histograms.signals.push_back(new TH2D(Form("histMass_signals%zu",i+1), Form("%.0f < p_{T,D^{0}} < %.1f GeV/c;#it{M}(K#pi) GeV/c^{2};#DeltaR",ptHFBinEdges[i],ptHFBinEdges[i+1]), massBins, minMass, maxMass, deltaRBinEdges.size() - 1, deltaRBinEdges.data()));
                // signals+reflections histograms: calculate ratios
                histograms.signals_and_reflections.push_back(new TH2D(Form("histMass_signals_and_reflections%zu",i+1), Form("%.0f < p_{T,D^{0}} < %.1f GeV/c;#it{M}(K#pi) GeV/c^{2};#DeltaR",ptHFBinEdges[i],ptHFBinEdges[i+1]), massBins, minMass, maxMass, deltaRBinEdges.size() - 1, deltaRBinEdges.data()));
        
            } else {
                // pure reflections histograms
                histograms.reflections.push_back(new TH2D(Form("histMass_reflections%zu",i+1), Form("%.0f < p_{T,D^{0}} < %.0f GeV/c;#it{M}(K#pi) GeV/c^{2};#DeltaR",ptHFBinEdges[i],ptHFBinEdges[i+1]), massBins, minMass, maxMass, deltaRBinEdges.size() - 1, deltaRBinEdges.data()));
                // pure signals histograms
                histograms.signals.push_back(new TH2D(Form("histMass_signals%zu",i+1), Form("%.0f < p_{T,D^{0}} < %.0f GeV/c;#it{M}(K#pi) GeV/c^{2};#DeltaR",ptHFBinEdges[i],ptHFBinEdges[i+1]), massBins, minMass, maxMass, deltaRBinEdges.size() - 1, deltaRBinEdges.data()));
                // signals+reflections histograms: calculate ratios
                histograms.signals_and_reflections.push_back(new TH2D(Form("histMass_signals_and_reflections%zu",i+1), Form("%.0f < p_{T,D^{0}} < %.0f GeV/c;#it{M}(K#pi) GeV/c^{2};#DeltaR",ptHFBinEdges[i],ptHFBinEdges[i+1]), massBins, minMass, maxMass, deltaRBinEdges.size() - 1, deltaRBinEdges.data()));
        ...
        if ((abs(MCDhfEta) < etaCut) && (abs(MCDhfY) < yCut) && ((MCDjetPt >= jetptMin) && (MCDjetPt < jetptMax)) && ((deltaR >= binning.deltaRBinEdges_detector[0]) && (deltaR < DeltaRcut))) {
            ...
            for (size_t iEdge = 0; iEdge < binning.ptHFBinEdges_detector.size() - 1 && !filled; iEdge++) {
                if ((MCDhfPt >= binning.ptHFBinEdges_detector[iEdge]) && (MCDhfPt < binning.ptHFBinEdges_detector[iEdge + 1])) {
                    ...
                    if (((intToD0Species(MCDhfMatchedFrom)) != D0Species::NEITHER) && ((intToD0Species(MCDhfSelectedAs)) != D0Species::NEITHER)) {
                        if ((intToD0Species(MCDhfMatchedFrom)) == (intToD0Species(MCDhfSelectedAs))) {
