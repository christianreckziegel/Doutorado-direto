...
void fillMassHistograms(TFile* fDist, const std::vector<TH2D*>& histograms, double jetptMin, double jetptMax, std::vector<double>& ptDBinEdges) {
    ...
    const double HfPtMincut = 3.; // on particle level
    const double HfPtMaxcut = 30.; // on particle level
...
SubtractionResult SideBand(const std::vector<TH2D*>& histograms2d, const std::vector<TF1*>& fittings, int signalSigmas, int startingBackSigma, int backgroundSigmas){
    ...
    for (size_t iHisto = 0; iHisto < vectorOutputs.histograms.size(); ++iHisto) {
        ...
        double leftSB = fittings[backgroundHist]->Integral(m_0 - (backgroundSigmas + startingBackSigma) * sigma,m_0 - startingBackSigma * sigma);
        ...
        tempHist = histograms2d[iHisto]->ProjectionY(Form("h_sideband_proj_temp_%zu", iHisto), lowBin, highBin); // right sideband
...
void PlotHistograms(const std::vector<TH2D*>& histograms2d, const std::vector<TF1*>& fittings, SubtractionResult outputStruct, double jetptMin, double jetptMax) {
    ...
    c1d_fit->Update();
    c1d_fit->SaveAs(imagePath + "SB_BackSub_invariant_mass" + jetPtRange + ".png");
    c_2d->Update();
    c_2d->SaveAs(imagePath + "SB_BackSub_2d_deltaR_vs_invmass" + jetPtRange + ".png");
    cSigPlusBack->Update();
    cSigPlusBack->SaveAs(imagePath + "SB_BackSub_yield_pT_bins" + jetPtRange + ".png");
    cAllPt->Update();
    cAllPt->SaveAs(imagePath + "SB_BackSub_yield_pT_summed" + jetPtRange + ".png");

    //
    // Storing in a single pdf file
    //
    c1d_fit->Print(Form("sb_subtraction_deltaR_%s.pdf(",jetPtRange.Data()));
    c_2d->Print(Form("sb_subtraction_deltaR_%s.pdf",jetPtRange.Data()));
    cSigPlusBack->Print(Form("sb_subtraction_deltaR_%s.pdf",jetPtRange.Data()));
...
void UpdateFinalDistribution(TH2D* hDeltaR_vs_ptJet, SubtractionResult& outputStruct, double& jetptMin) {
    
    // Create the sum of pT,D bins distribution
    TH1D* hSumDeltaR = (TH1D*)outputStruct.subtractedHist[0]->Clone("hSumDeltaR");
    ...
    int binY = hDeltaR_vs_ptJet->GetYaxis()->FindBin(jetptMin);
...
void ClearData(SubtractionResult& outputStruct, std::vector<TH2D*>& massHistograms) {
    ...
    for (auto& hist : outputStruct.histograms) {
        delete hist;
    }
    outputStruct.histograms.clear();

    for (auto& hist : outputStruct.sidebandHist) {
        delete hist;
    }
    outputStruct.sidebandHist.clear();

    for (auto& hist : outputStruct.signalHist) {
        delete hist;
    }
    outputStruct.signalHist.clear();

    for (auto& hist : outputStruct.subtractedHist) {
        delete hist;
    }
    outputStruct.subtractedHist.clear();

    if (outputStruct.hSubtracted_allPtSummed != nullptr) {
        delete outputStruct.hSubtracted_allPtSummed;
        outputStruct.hSubtracted_allPtSummed = nullptr;
    }
    
    // Delete mass histograms
...
void AnalyzeJetPtRange(TFile* fDist, TH2D* hDeltaR_vs_ptJet, std::vector<double>& ptDBinEdges, std::vector<double>& deltaRBinEdges, double& jetptMin, double& jetptMax, TFile* fOut) {
    ...
    std::vector<TH2D*> massHistograms = createHistograms(names, titles, 
    ...
    cout << "DeltaR bin width = " << (deltaRBinEdges[deltaRBinEdges.size() - 1]-deltaRBinEdges[0])/(deltaRBinEdges.size() - 1) << endl;

    

    // Fill histograms
    fillMassHistograms(fDist, massHistograms, jetptMin, jetptMax, ptDBinEdges);

    // Perform fits
    std::vector<TF1*> fittings = performFit(names,massHistograms, parametersVectors, minMass, maxMass);
    ...
    SubtractionResult outputStruct = SideBand(massHistograms, fittings, signalSigmas, startingBackSigma, backgroundSigmas);
    
    // Plot histograms
    PlotHistograms(massHistograms, fittings, outputStruct, jetptMin, jetptMax);
...
void BackgroundSubtraction_2D() {
    ...
    if (!fDist || fDist->IsZombie()) {
        std::cerr << "Error: Unable to open the data ROOT file." << std::endl;
    }
    TFile* fOut = new TFile(Form("backSub_%d_to_%d_jetpt_2D.root",static_cast<int>(ptjetBinEdges[0]),static_cast<int>(ptjetBinEdges[ptjetBinEdges.size() - 1])),"recreate");
    if (!fOut || fOut->IsZombie()) {
        std::cerr << "Error: Unable to create the output ROOT file." << std::endl;
    }
    // Create final histogram with multiple pT,jet ranges
    TH2D* hDeltaR_vs_ptJet = new TH2D("hDeltaR_vs_ptJet","Background subtracted data;#DeltaR;p_{T,jet}", deltaRBinEdges.size() -1, deltaRBinEdges.data(), ptjetBinEdges.size() - 1, ptjetBinEdges.data());

    // Loop over all the pT,jet ranges
    for (size_t iPtRange = 0; iPtRange < ptjetBinEdges.size() - 1; iPtRange++) {
        AnalyzeJetPtRange(fDist, hDeltaR_vs_ptJet, ptDBinEdges, deltaRBinEdges, ptjetBinEdges[iPtRange], ptjetBinEdges[iPtRange + 1], fOut);
    }
