...
struct HfFragmentationFunctionTask {
  ...
  Preslice<JetMCPTable> D0MCPJetsPerMCCollision = aod::jet::collisionId;
  ...
  void init(InitContext const&)
  {
    ...
    // D0 candidate histograms from MC simulation
    registry.add("h_mcp_d0_jet_pt", ";#p_{T,D^{0} jet}^{truth} (GeV/c);dN/d#p_{T,D^{0} jet}^{truth}", {HistType::kTH1F, {{1000, 0., 40.}}});
    registry.add("h_mcd_d0_jet_pt", ";#p_{T,D^{0} jet}^{detector} (GeV/c);dN/d#p_{T,D^{0} jet}^{detector}", {HistType::kTH1F, {{1000, 0., 40.}}});
  }
                                      ...
                                      CandidatesD0Data const&)
  {
    ...
    for (auto& jet : jets) {
      ...
      for (auto& d0Candidate : jet.hfcandidates_as<CandidatesD0Data>()) { // for jet constituents use -> auto& jetConstituent : jet.tracks_as<JetTracks>()
        ...
        axisDistance = sqrt(pow(DeltaPhi(jet.eta(),d0Candidate.eta()), 2) + pow(jet.phi() - d0Candidate.phi(), 2));
                               ...
                               JetTracks const& tracks, JetParticles const& particles) 
  {
    ...
    for (auto& mccollision : mccollisions) {
      ...
      // loop over reconstructed collisions associated to same mccollision
      ...
      for (auto& collision : collisionsPerMCCollision) {
        // loop over d0 detector level jets associated to same collision
        const auto d0mcdJetsPerCollision = mcdjets.sliceBy(D0MCDJetsPerCollision, collision.globalIndex());
        
        if (d0mcdJetsPerCollision.size() != 0) {
          std::cout << "There were " << d0mcdJetsPerCollision.size() << " mcd jets\n"; 
        }
        ...
        for (auto& mcdjet : d0mcdJetsPerCollision) {
          ...
          // calculating angular distance in eta-phi plane
          axisDistance = sqrt(pow(DeltaPhi(mcdjet.eta(),mcdd0cand.eta()), 2) + pow(mcdjet.phi() - mcdd0cand.phi(), 2));
      ...
      // loop over d0 particle level jets associated to same mccollision
      ...
      for (auto& mcpjet : d0mcpJetsPerMCCollision) {
        ...
        // calculating angular distance in eta-phi plane
        axisDistance = sqrt(pow(DeltaPhi(mcpjet.eta(),mcpd0cand.eta()), 2) + pow(mcpjet.phi() - mcpd0cand.phi(), 2));
        
        // store data in MC detector level table
        mcpdistJetTable(axisDistance, mcpjet.pt(), mcpjet.eta(), mcpjet.phi(), mcpd0cand.pt(), mcpd0cand.eta(), mcpd0cand.phi(), jethfutilities::getTablePDGMass<CandidatesD0MCP>(), mcpd0cand.y(),mcpjet.has_matchedJetCand());
